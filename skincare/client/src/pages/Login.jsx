...
import { authService, skinService } from '../services/api';
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userObj = { username: userData.username, id: userData.userId, token: userData.token };
      login(userObj);

      // Check if user already has an analysis
      try {
        const currentPlan = await skinService.getCurrentPlan(userData.userId);
        if (currentPlan && currentPlan.skinType) {
          navigate('/dashboard');
        } else {
          navigate('/language');
        }
      } catch (err) {
        // If error fetching plan (e.g. 404), go to language/analysis
        navigate('/language');
      }
    } catch (err) {
